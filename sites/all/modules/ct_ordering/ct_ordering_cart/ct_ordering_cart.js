(function ($, Drupal, window, document, undefined) {
  ...
  function AddToShoppingCart(id, name, num, price) {
    ...
    if ($.cookie('order_info') == null) {
      $.cookie('order_info', jsonStr, cookieSet);
    }
    else {
      var jsonObj = eval('(' + $.cookie('order_info') + ')'); //将json字符串转换成数组
      var findFood = false;
      ...
      $.cookie('order_info', JSON.stringify(jsonObj), cookieSet); //需要json2.js支持
    }
  }
